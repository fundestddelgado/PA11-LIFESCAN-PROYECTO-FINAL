...
function inicializarChat() {
    ...
    mostrarMensajeBot('¡Hola! Soy tu asistente IA de salud. Puedo ayudarte a interpretar resultados, explicar términos médicos o responder preguntas generales sobre salud. ¿En qué puedo asistirte hoy?');
}
...
function enviarMensaje() {
    ...
    setTimeout(() => {
        const respuestasIA = [
            'Entiendo tu consulta. Basado en la información disponible, te recomendaría consultar con un profesional médico para una evaluación completa de tu situación específica.',
            'Es importante recordar que aunque la IA puede proporcionar información general, no puede diagnosticar condiciones médicas. ¿Hay algún síntoma específico que te preocupe?',
            'Para brindarte información más precisa, necesitaría más contexto sobre tu consulta. ¿Podrías proporcionar más detalles?',
            'Basado en patrones comunes, puedo sugerirte que mantengas hábitos saludables como ejercicio regular, dieta balanceada y sueño adecuado.',
            'Tu pregunta es importante. Te recomiendo llevar esta consulta a un médico quien podrá evaluarte personalmente.',
            'La prevención es clave en salud. Asegúrate de realizar chequeos médicos regulares según tu edad y factores de riesgo.'
        ];
        
        const respuestaAleatoria = respuestasIA[Math.floor(Math.random() * respuestasIA.length)];
        mostrarMensajeBot(respuestaAleatoria);
    }, 1000);
}
...
function mostrarMensajeBot(texto) {
    ...
    mensajeDiv.innerHTML = `<strong>Asistente IA:</strong> ${texto}`;
    messagesContainer.appendChild(mensajeDiv);
    messagesContainer.scrollTop = messagesContainer.scrollHeight;
}
